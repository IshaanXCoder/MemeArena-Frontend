      ...
      >
        ...
        >
          ...
          {stakeInfo.userHasStaked && (
            <div className="absolute top-3 left-3 bg-mustard text-charcoal px-2 py-1 border-2 border-charcoal font-black text-xs uppercase shadow-md">
          ...
          {!stakeInfo.isStakeable && stakeInfo.stakeDelay > 0 && (
            <div className="absolute bottom-3 left-3 bg-orange text-beige px-2 py-1 border-2 border-orange-dark font-black text-xs uppercase flex items-center gap-1 shadow-md">
        ...
        <motion.div 
          className="p-4 bg-beige"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.2, duration: 0.3 }}
        >
          <motion.h3 
            className="font-black text-xl mb-3 uppercase tracking-tight text-charcoal"
          ...
          <div className="flex items-center gap-3 mb-4">
            <Avatar className="h-10 w-10 border-2 border-teal">
              ...
              <AvatarFallback className="bg-teal text-beige font-bold">{meme.creator.name[0]}</AvatarFallback>
            </Avatar>
            <span className="text-sm font-bold text-charcoal-light uppercase">{meme.creator.name}</span>
          </div>

          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2 bg-mustard text-charcoal px-3 py-1 border-2 border-charcoal font-black shadow-md">
            ...
            <Link href={`/meme/${meme.id}`}>
              <motion.div
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="bg-orange text-beige p-2 border-2 border-orange-dark hover:bg-orange-light transition-colors shadow-md"
              >
